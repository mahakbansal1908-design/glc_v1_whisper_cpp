...
def _resolve_cli() -> str | None:
    ...
    #   we look for the binary in two places — an env var for when
    # you've installed whisper.cpp somewhere weird, and then the normal PATH.
    # Beats hardcoding /usr/local/bin and having it break on someone else's machine.
...
def _resolve_model() -> Path | None:
    ...
    """
    #   first check if someone set GLC_WHISPER_MODEL_DIR. We try both
    # ggml-base.bin and ggml-base.en.bin because the English-only model is
    # smaller and faster — but not everyone has it downloaded.
    ...
    #   fall back to the default model location. WHISPER_MODEL env var
    # lets you switch between tiny/base/small without changing code paths.
...
def _find_wav_chunk(audio: bytes, chunk_id: bytes) -> tuple[int, int] | None:
    ...
    #   wave.open from the stdlib is great, but it chokes on WAV files
    # that have extra LIST chunks (common in screen recordings). Walking the
    # RIFF tree manually handles everything — LIST, fact, cue, whatever.
    if len(audio) < 12 or audio[:4] != b"RIFF":
        return None
    # skip RIFF header + file size + WAVE ID
    pos = 12
    while pos + 8 <= len(audio):
        ...
        # Skip the chunk content plus padding.
        pos += 8 + size + (size % 2)
    return None
...
def _trim_silence_wav(audio: bytes, min_silence_ms: int = 3000) -> bytes:
    ...
    """
    #   _trim_silence_wav removes silence BEFORE whisper sees it.
    # whisper's built-in VAD runs inside the subprocess, after startup is paid.
    # Trimming first means less audio → faster inference. Win-win.
    ...
    if bits_per != 16 or channels != 1:
        return audio  #   only 16-bit mono for now
    ...
    #   collapse runs of silence > min_silence_ms
    ...
    #   build output as bytearray to avoid huge list[int]
    out = bytearray()
...
def _extract_duration_ms(data: dict) -> int:
    ...
    #   whisper-cli has changed its JSON output format a few times.
    # Sometimes it's "transcription", sometimes "segments". Same for timestamps —
    # "end", "t1", "offsets.to" all mean the same thing but appear in different
    # versions. We just try them all.
...
def _parse_stderr_transcript(stderr: str) -> tuple[str, int]:
    ...
    """
    #   when -vth (VAD) is on, whisper-cli doesn't always write the
    # JSON sidecar file. Instead it prints the transcript to stderr with
    # timestamps. If we don't parse this, VAD-enabled audio returns empty text.
    import re
...
def run_whisper_cpp(audio: bytes, mime: str, use_vad: bool = False) -> tuple[str, str, int]:
    ...
    #   skip silence trim when VAD already handles it
    ...
    #   -vth enables whisper's built-in VAD. It drops non-speech
    # segments from the output, which saves decoding time on long audio.
    # The repo had -nth which is a different flag — -vth is the right one.
    if use_vad:
        cmd.extend(["-vth", VAD_THRESHOLD])

    #   timeout prevents a hung subprocess from hanging the whole
    # pipeline. 300 seconds is generous — even a 30-minute audio file won't
    ...
    #   when VAD is enabled or JSON sidecar is absent, whisper-cli
    # prints the transcript to stderr (not stdout) in timestamped lines:
    #   [00:00:00.000 --> 00:00:07.000]   some text here
    # Parse those lines to recover the text and duration.
